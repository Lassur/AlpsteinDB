...
// === Neuer API-Endpunkt für dynamisch gefilterte Einträge ===
const express = require('express');
const router = express.Router();
const Eintrag = require('./models/Eintrag'); // Du musst sicherstellen, dass das Modell existiert und korrekt ist
...
app.get('/api/eintraege', async (req, res) => {
  try {
    ...
    if (gipfel) match.Gipfel = gipfel;
    if (route) match.Route = route;
    if (buch) match.Buch = buch;
    if (erstbegehung) match.Erstbegehung = erstbegehung;
    if (jahrVon || jahrBis) {
      match.Jahr = {};
      if (jahrVon) match.Jahr.$gte = parseInt(jahrVon);
      if (jahrBis) match.Jahr.$lte = parseInt(jahrBis);
    }
    if (wer) match.Wer = { $regex: wer, $options: 'i' };
    if (notiz) match.Notiz = { $regex: notiz, $options: 'i' };
          ...
          filters: [
            {
              $group: {
                _id: null,
                gipfel: { $addToSet: '$Gipfel' },
                route: { $addToSet: '$Route' },
                buch: { $addToSet: '$Buch' },
                erstbegehung: { $addToSet: '$Erstbegehung' }
              }
            }
          ]
        }
    ...
    const result = await Eintrag.aggregate(pipeline);
    const response = {
      data: result[0].data,
      total: result[0].totalCount[0]?.count || 0,
      page: parseInt(page),
      limit: parseInt(limit),
      filters: result[0].filters[0] || {}
    };

    res.json(response);
  } catch (error) {
    console.error('Fehler bei /api/eintraege:', error);

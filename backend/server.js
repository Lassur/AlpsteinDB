...
// Chart: Top 10 Tage
app.get("/api/chart/top10tage", async (req, res) => {
  try {
    await client.connect();
    const collection = client.db(dbName).collection(collectionName);
    ...
    const output = result.map(r => {
      ...
      const formatted = new Intl.DateTimeFormat("de-DE", options).format(dateObj);
      return { datum: formatted, count: r.count };
    });

    res.json(output);
  } catch (err) {
    console.error("Fehler bei /api/chart/top10tage", err);
    res.status(500).json({ error: err.message });
  }
});
...
// NEU: Chart Top 10 Tage (vollständig clientseitig gezählt)
app.get("/api/chart/top10tage", async (req, res) => {
  try {
    await client.connect();
    const collection = client.db(dbName).collection(collectionName);
    ...
    // Alle Einträge mit gültigem Datum_JMT
    const eintraege = await collection.find(filter, { projection: { Datum_JMT: 1 } }).toArray();
    ...
    eintraege.forEach(e => {
      ...
      if (typeof d === "string" && /^\d{4}-\d{2}-\d{2}$/.test(d)) {
        zaehler[d] = (zaehler[d] || 0) + 1;
      }
    });

    // Top 10 berechnen
    const top10 = Object.entries(zaehler)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10)
      .map(([datum, count]) => {
        ...
        return {
          datum: formatter.format(dateObj),
          count
        };
      });
    ...
    console.error("Fehler bei NEU /api/chart/top10tage", err);
    res.status(500).json({ error: err.message });
  }
});

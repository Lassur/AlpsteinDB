...
// Neue API-Route: Anzahl Einträge pro Gipfel
app.get('/api/eintraege-pro-gipfel', async (req, res) => {
  try {
    await client.connect();
    const db = client.db('GipfelbuchDatabase'); // ggf. anpassen
    const collection = db.collection('GipfelbuchCollection'); // ggf. anpassen

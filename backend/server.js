...
// API: Bücher im gewählten Zeitraum
app.get("/api/buecher", async (req, res) => {
  try {
    await client.connect();
    const collection = client.db(dbName).collection(collectionName);

    const von = parseInt(req.query.von) || 1900;
    const bis = parseInt(req.query.bis) || 2100;

    const result = await collection.aggregate([
      {
        $match: {
          Jahr: { $gte: von, $lte: bis },
          Buch: { $exists: true, $ne: null }
        }
      },
      { $group: { _id: "$Buch" } },
      { $sort: { _id: 1 } }
    ...
    res.json(result.map(d => d._id));
  } catch (err) {
    console.error("Fehler bei /api/buecher", err);
    res.status(500).json({ error: err.message });
  }
});

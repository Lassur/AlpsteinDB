...
// Chart: Einträge nach Routen (wird von deinem Frontend verwendet)
...
// **NEUER ENDPOINT**: Einträge nach Routen über die Jahre (wird von deinem Frontend verwendet)
app.get("/api/chart/route_over_years", async (req, res) => {
  try {
    await client.connect();
    const collection = client.db(dbName).collection(collectionName);
    ...
    console.error("Fehler bei /api/chart/route_over_years", err);
    res.status(500).json({ error: err.message });
  }
});

...
app.get("/api/chart/top10tage", async (req, res) => {
  try {
    ...
    const filter = {};
    const { Jahr, Monat, Gipfel, Route } = req.query;
    if (Jahr) filter.Jahr = Jahr;
    if (Monat) filter.Monat = Monat;
    ...
    for (const doc of docs) {
      ...
      const label = dateObj.toLocaleDateString("de-DE", {
        weekday: "long",
        day: "2-digit",
        month: "2-digit",
        year: "numeric",
    ...
    const sorted = Object.entries(countByDate)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10)
    ...
    console.error(err);
    res.status(500).send("Fehler beim Abrufen der Top-Tage");
  }
});
app.listen(PORT, () => {

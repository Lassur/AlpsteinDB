...
app.get("/api/chart/jahr", async (req, res) => {
  try {
    ...
    if (req.query.jahr_von || req.query.jahr_bis) {
        filter.Jahr = {};
        if (req.query.jahr_von) filter.Jahr.$gte = parseInt(req.query.jahr_von);
        if (req.query.jahr_bis) filter.Jahr.$lte = parseInt(req.query.jahr_bis);

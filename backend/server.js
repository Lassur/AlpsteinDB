...
app.get("/api/chart/top10tage", async (req, res) => {
  try {
    ...
    const pipeline = [
      ...
      {
        $group: {
          _id: "$Datum", // Gruppierung nach Datum
          count: { $sum: 1 }
        }
      },
      { $sort: { count: -1 } },
    ...
    console.error("Fehler bei /chart/top10tage:", err);

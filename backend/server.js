...
async function firebaseAuthMiddleware(req, res, next) {
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return res.status(401).json({ error: "Kein oder ungültiger Authorization-Header" });
  }
...
// Einträge-Endpunkt
app.get("/api/eintraege", firebaseAuthMiddleware, async (req, res) => {
  try {
  ...
  if (req.query[f]) {
    ...
    // Erweiterung: Jahr von-bis Filter
    ...
    // Erweiterung: Wer-Filter (Textsuche)
    if (req.query.wer) {
      filter.Wer = { $regex: req.query.wer, $options: "i" };
    }

    // Erweiterung: Notiz-Filter (Textsuche)
    if (req.query.notiz) {
      filter.Notiz = { $regex: req.query.notiz, $options: "i" };
    }
...
app.get("/api/chart/jahr", async (req, res) => {
  ...
  const bis = parseInt(req.query.bis) || 9999;
  filter.Jahr = { $gte: von, $lte: bis };
} else if (req.query.jahr) {
  filter.Jahr = parseInt(req.query.jahr);
}
...
// Chart: Top 10 Tage – V2 (Datum_JMT basiert)
app.get("/api/chart/top10tage_v2", async (req, res) => {
  ...
  filter.Jahr = parseInt(req.query.jahr);
    ...
    const output = daten.map(r => {
      const dateObj = new Date(r._id + "T00:00:00");
...
// Momente: Heute vor X Jahren – robustes Tag/Monat-Matching
app.get("/api/momente/heute", async (req, res) => {
  try {
    ...
    const tagMonat = `-${monat}-${tag}`;

    const result = await collection.find({
      Datum_JMT: { $regex: `${tagMonat}$` },
      Notiz: { $exists: true, $ne: "" }
    }).toArray();

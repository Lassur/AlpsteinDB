...
// Gemeinsamer Dropdown-Filter-Endpunkt
app.get("/api/filter-options", async (req, res) => {
  try {
    ...
    const filter = {};
    if (req.query.erstbegehung) filter.Erstbegehung = req.query.erstbegehung;
    if (req.query.jahrVon) filter.Jahr = { ...filter.Jahr, $gte: parseInt(req.query.jahrVon) };
    if (req.query.jahrBis) filter.Jahr = { ...filter.Jahr, $lte: parseInt(req.query.jahrBis) };
    if (req.query.gipfel) filter.Gipfel = req.query.gipfel;
    if (req.query.route) filter.Route = req.query.route;
    if (req.query.buch) filter.Buch = req.query.buch;

    const [gipfel, route, buch] = await Promise.all([
      collection.distinct("Gipfel", filter),
      collection.distinct("Route", filter),
      collection.distinct("Buch", filter),
    ]);

    res.json({
      gipfel: gipfel.filter(Boolean).sort(),
      route: route.filter(Boolean).sort(),
      buch: buch.filter(Boolean).sort()
    });
  } catch (err) {
    console.error("Fehler bei /api/filter-options", err);
    res.status(500).json({ error: err.message });
  }
});

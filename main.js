...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getFilterParams(excludeField = null) {
    ...
    for (const [key, el] of Object.entries(filters)) {
      ...
      if (value && paramName !== "wer") {
        params.append(paramName, value);
      }
    }

    if (filters.Wer.value && excludeField !== "Wer") {
  ...
  function loadFilters() {
    ...
    all.forEach(field => {
      ...
      fetch(`${apiUrl}/werte?feld=${encodeURIComponent(field)}&${params.toString()}`)
        ...
        if (field === "Jahr") {
            const jahrOptions = [...new Set(data)].sort((a, b) => a - b);
            ["VonJahr", "BisJahr"].forEach(id => {
              const sel = filters[id];
              const selected = sel.value;
              sel.innerHTML = `<option value="">${id === "VonJahr" ? "Von Jahr" : "Bis Jahr"}</option>`;
              jahrOptions.forEach(y => {
                const opt = new Option(y, y);
                sel.add(opt);
              });
              // Wenn der alte Wert noch existiert, wieder setzen
              if (selected && jahrOptions.includes(parseInt(selected))) {
                sel.value = selected;
              }
            });

...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getFilterParams(exclude = null) {
    const params = new URLSearchParams();

    filterFields.forEach(f => {
      if (f !== exclude) {
        const val = filters[f].value;
        if (val) params.append(f.toLowerCase(), val);
      }
    });

    if (exclude !== "Jahr") {
      if (filters.VonJahr.value) params.append("von", filters.VonJahr.value);
      if (filters.BisJahr.value) params.append("bis", filters.BisJahr.value);
    }

    if (exclude !== "Wer" && filters.Wer.value) {
      params.append("wer", filters.Wer.value);
    }

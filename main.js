...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Warte auf DataTable und DOM
    function addColumnSearchInputs() {
      if (!window.myDataTable || !document.querySelector("#gipfeltabelle thead")) {
        setTimeout(addColumnSearchInputs, 100);
        return;
      }
      ...
      // Falls bereits eingefügt, nicht erneut
      if (thead.rows.length > 1) return;
      ...
      for (let i = 0; i < headerRow.cells.length; i++) {
        ...
        input.addEventListener("keyup", function () {
          window.myDataTable.columns().search(input.value, i);
        });
    ...
    // Starte die Überwachung kurz nach dem Init
    setTimeout(addColumnSearchInputs, 500);

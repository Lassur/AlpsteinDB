...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getFilterParams(excludeField = null) {
    ...
    filterFields.forEach(field => {
      ...
      if (field !== excludeField && val) {
        // Correctly pass the "Erstbegehung" values ("Ja", "Nein", or "Ja (Unterkategorie von Erstbegehung)")
        if (field === "Erstbegehung") {
          if (val === "Ja") {
            params.append(field.toLowerCase(), "Ja");  // "Ja" as "Ja"
          } else if (val === "Nein") {
            params.append(field.toLowerCase(), "Nein"); // "Nein" as "Nein"
          } else if (val === "Ja (Unterkategorie von Erstbegehung)") {
            params.append(field.toLowerCase(), "Ja (Unterkategorie von Erstbegehung)"); // Special handling for subcategory
          }
        } else {
          params.append(field.toLowerCase(), val); // Treat other filters normally
        }
      }
    });
    return params;
  ...
  async function fetchData() {
    ...
    // Debugging: log the final parameters sent to the API

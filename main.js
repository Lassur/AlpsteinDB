...
function applyFiltersFromURL() {
  ...
  const gipfel = params.get("gipfel");
  const von = params.get("von");
  const bis = params.get("bis");

  if (gipfel) {
    const gipfelSelect = document.getElementById("filterGipfel");
    if (gipfelSelect) gipfelSelect.value = gipfel;
  }
  if (von) {
    const vonSelect = document.getElementById("filterVonJahr");
    if (vonSelect) vonSelect.value = von;
  }
  if (bis) {
    const bisSelect = document.getElementById("filterBisJahr");
    if (bisSelect) bisSelect.value = bis;
  }

  // Lade Einträge nach kleinem Timeout, damit Dropdowns geladen sind
  setTimeout(() => {
    fetchDataIfFiltersSet();
  }, 500);
}
...
function applyFiltersFromURL(retries = 10) {
  const params = new URLSearchParams(window.location.search);
  const gipfel = params.get("gipfel");
  const von = params.get("von");
  const bis = params.get("bis");
  ...
  const dropdownsReady =
    gipfelSelect && gipfelSelect.options.length > 1 &&
    vonSelect && vonSelect.options.length > 1 &&
    bisSelect && bisSelect.options.length > 1;

  if (!dropdownsReady && retries > 0) {
    setTimeout(() => applyFiltersFromURL(retries - 1), 300);
    return;
  }

  if (gipfel) gipfelSelect.value = gipfel;
  if (von) vonSelect.value = von;
  if (bis) bisSelect.value = bis;

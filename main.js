document.addEventListener("DOMContentLoaded", () => {
  ...
  excelButton.addEventListener("click", () => {
    ...
    if (table) {
      ...
      // Add the reference and timestamp in the header
      const date = new Date().toLocaleString();
      const header = `Andreas Koller – Projekt 'AlpsteinDB', https://lassur.github.io/AlpsteinDB/ \nExportiert am: ${date}`;
      
      // Insert the header into the first row of the Excel file
      const ws = wb.Sheets["Data"];
      const cell = ws["A1"];
      ws["A1"] = header;

      // Set the title for the first column if it's missing
      if (!ws["A1"]) ws["A1"] = "Gipfel";
  ...
  printButton.addEventListener("click", () => {
    ...
    if (table) {
      ...
      printWindow.document.write(`
        body { font-family: Arial, sans-serif; margin: 0; padding: 20px; font-size: 0.85rem; }

...
document.addEventListener("DOMContentLoaded", () => {
  ...
  loadFilters(); // Nur Filter initial laden
  hideLoadingSpinner();
  ...
  function getFilterParams(excludeField = null) {
    ...
    filterFields.forEach(field => {
      if (field !== excludeField && filters[field]) {
...
function loadFilters() {
  [...filterFields, "Jahr"].forEach(field => {
    ...
    if (field === "Jahr") {
      // Nur von/bis entfernen, damit die beiden sich nicht gegenseitig einschränken
      params.delete("von");
      params.delete("bis");
    } else {
      // Alle anderen Felder sollen bei normalen Dropdowns von Jahr entkoppelt sein
      params.delete("von");
      params.delete("bis");
      if (field !== "Wer") {
        params.delete("wer");

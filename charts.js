...
async function updateDropdowns(primaryFilters = {}, skipGipfel = false) {
  const felder = {
    ...
    filterErfasste_Jahre: "Erfasste_Jahre",  // Erfasste_Jahre Filter bleibt erhalten
    filterJahr: "Jahr"
  };
  ...
  for (const [id, feldname] of Object.entries(felder)) {
    ...
    // Den Dropdown-Wert nicht zurücksetzen, sondern nur Optionen hinzufügen/aktualisieren
    const defaultOption = el.querySelector("option");
    
    // Optionen löschen, aber Standardwert beibehalten
    const options = Array.from(el.options);
    options.forEach(opt => el.removeChild(opt));

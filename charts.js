...
const felder = {
  ...
  // Sicherstellen: Gipfel-Wert fallback über selectedIndex
  if (!filter.gipfel) {
    const sel = document.getElementById("filterGipfel");
    if (sel && sel.selectedIndex >= 0) {
      const fallback = sel.options[sel.selectedIndex]?.value;
      if (fallback) filter.gipfel = fallback;
    }
  }
...
function getFilterParamsOhne(ausschlussId) {
  const params = new URLSearchParams();
  for (const [htmlId, feldname] of Object.entries(felder)) {
    if (htmlId === ausschlussId) continue;
    const el = document.getElementById(htmlId);
    const val = el?.value;
    if (val) params.append(feldname.toLowerCase(), val);
  }

  const von = document.getElementById("filterVonJahr")?.value;
  const bis = document.getElementById("filterBisJahr")?.value;
  if (ausschlussId !== "filterVonJahr" && von) params.append("von", von);
  if (ausschlussId !== "filterBisJahr" && bis) params.append("bis", bis);
  return params;
}

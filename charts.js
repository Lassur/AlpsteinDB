...
let chart, saisonChart, routeChart, tageChart;
...
async function fetchAndRenderRouteChart(filters) {
  ...
  const colors = labels.map((_, i) =>
    `hsl(${(i * 360) / labels.length}


async function fetchAndRenderTageChart(filters) {
  ...
  const ctx = document.getElementById("tageChart").getContext("2d");
  tageChart = new Chart(ctx, {
    type: "bar",
    data: {
      labels,
      datasets: [{
...
async function initialisieren() {
  ...
  fetchAndRenderTageChart(filters);
}

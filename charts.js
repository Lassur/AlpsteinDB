...
async function fetchAndRenderTopdayChart(filters) {
    ...
    options: {
      ...
      plugins: {
        tooltip: {
          callbacks: {
            label: function(ctx) {
              return `${ctx.label}: ${ctx.raw} Einträge`;
            }
          }
        }
      },
  ...
  if (window.topdayChart) window.topdayChart.destroy();
  window.topdayChart = new Chart(document.getElementById("topdayChart"), config);
}
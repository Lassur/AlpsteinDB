...
let chart, saisonChart, routeChart, routeOverYearsChart;
...
async function fetchAndRenderRouteOverYearsChart(filters) {
  const res = await fetch(buildUrl(filters, "route_over_years"));
  const data = await res.json();

  const jahre = Array.from(new Set(data.map(d => d._id)));
  const routes = Array.from(new Set(data.flatMap(d => d.routes.map(route => route.name))));

  const datasets = routes.map(route => {
    const routeData = new Array(jahre.length).fill(0);
    data.forEach(d => {
      const routeEntry = d.routes.find(r => r.name === route);
      const yearIndex = jahre.indexOf(d._id);
      if (routeEntry) routeData[yearIndex] = routeEntry.count;
    });
    ...
    options: {
      ...
      plugins: {
        tooltip: {
          callbacks: {
            label: function (ctx) {
              return `${ctx.dataset.label}: ${ctx.raw} Einträge`;
            }
          }
        }
      },
  ...
  if (routeOverYearsChart) routeOverYearsChart.destroy();
  routeOverYearsChart = new Chart(document.getElementById("routeOverYearsChart"), config);
}

async function fetchAndRenderAllCharts(filters) {
  await fetchAndRenderChart(filters);          // Bestehendes Jahr-Diagramm
  await fetchAndRenderSaisonChart(filters);    // Bestehendes Saison-Diagramm
  await fetchAndRenderRouteChart(filters);     // Bestehendes Routen-Diagramm
  await fetchAndRenderRouteOverYearsChart(filters); // Neues Diagramm für Routen über die Jahre
}
...
async function initialisieren() {
  ...
  await fetchAndRenderAllCharts(filters);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".filter-container select").forEach(sel => {
    sel.addEventListener("change", () => {
      ...
      fetchAndRenderAllCharts(filter);
    });
  });
});

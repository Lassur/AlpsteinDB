...
async function fetchAndRenderTop10TageChart(filters) {
  const res = await fetch(buildUrl(filters, "top10tage"));
  const data = await res.json();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function applyAllCharts() {
    ...
    fetchAndRenderChart(filters);
    fetchAndRenderSaisonChart(filters);
    fetchAndRenderRouteChart(filters);
    fetchAndRenderTop10TageChart(filters);
  }

  if (filterButton) {
    filterButton.addEventListener("click", applyAllCharts);
  }

  filterSelects.forEach(select => select.addEventListener("change", applyAllCharts));

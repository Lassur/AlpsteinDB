...
fetch('data.json')
  ...
  .then(data => {
    const tooltips = {
      ...
      "Doppeleintrag": "Teilweise haben sich Besteiger gleichzeitig in ein Wandbuch und oben angekommen in das Gipfelbuch eingetragen. Bsp. Direkte Nordwand, gen. \"Flugroute\", am Ersten Kreuzberg. Um die Daten zu normalisieren - sprich: den entsprechenden Besteiger nur 1mal zu erfassen - ist diese Spalte angelegt worden. Die Spalte dient statistischen Zwecken.",
      "Erstbegehung": "Erstbegehungen sind mit \"Ja\" gekennzeichnet. Ebenfalls sind Unterkategorien von Erstbegehungen, inkl. erste Winterbegehungen, erste Damenbesteigungen, erste Traversen, etc. gekennzeichnet."
    };
    ...
    const table = $('#gipfelTable').DataTable({
      ...
      initComplete: function () {
        ...
        // Tooltips korrekt anwenden
        $('#gipfelTable thead tr:eq(0) th').each(function(i) {
          const colKey = Object.keys(data[0])[i];
          const tip = tooltips[colKey];
          if (tip) {
            $(this).attr('title', tip);
